...
const thBase =
  "px-2.5 py-2 text-[9.5px] font-bold tracking-[0.5px] uppercase whitespace-nowrap";

export function CasesSlaWatchlist({
  scopeName,
  items,
  totalPastSla,
}: CasesSlaWatchlistProps) {
  ...
  const moreCases =
    totalPastSla && totalPastSla > items.length
      ? totalPastSla - items.length
      : 0;
  ...
  return (
    <div className="card mt-3.5" style={{ borderLeft: "3px solid var(--red)" }}>
      {/* ── HEADER ── */}
      <div className="card-title flex flex-col gap-1 lg:flex-row lg:items-center lg:justify-between">
        <span>
          🔥 SLA breach watchlist · {scopeName} · top {items.length}
          {totalPastSla && totalPastSla > items.length
            ? ` of ${formatIndian(totalPastSla)}`
            : ""}
        </span>
        <span className="text-[10px] md:text-[10.5px] font-normal text-text-dim lg:text-right">
          ranked by overdue days × est. value · {fmtINR(totalExposure)} combined
          exposure
        </span>
      </div>
      ...
      <div className="page-sub my-[-2px] mb-2.5 text-[10.5px]">
        ...
        Production wiring would auto-page next-level supervisor after 48h
        overdue.
      </div>

      {/* ── TABLE ── */}
      <div className="overflow-x-auto rounded-[7px]">
        <table className="min-w-[900px] w-full table-fixed border-collapse text-[11.5px]">
          <colgroup>
          ...
          <tbody>
            {items.map((c) => {
              const riskTier =
                c.risk >= 80 ? "high" : c.risk >= 60 ? "mid" : "low";
              const riskClasses =
                riskTier === "high"
              ...
              return (
                ...
                >
                  ...
                  <td className="p-2.5 overflow-hidden">
                    <div
                      className={`inline-flex h-8 w-8 items-center justify-center rounded-[7px] border-2 font-mono text-[11px] font-extrabold ${riskClasses}`}
                    >
                      {c.risk}
                    </div>
                  </td>
                  ...
                  <td className="p-2.5 overflow-hidden">
                    ...
                    <div className="mt-0.5 truncate text-[11.5px] font-semibold text-text">
                      {consumerShort}
                    </div>
                    <div className="mt-px truncate text-[9.5px] text-text-dim">
                      {c._activity} · {c._load}
                      {c._load_unit}
                    </div>
                  </td>
                  ...
                  {/* INSPECTOR */}
                  <td className="p-2.5 overflow-hidden text-[10.5px] text-text-mid">
                    {c.assignee || "—"}
                  </td>

                  {/* OVERDUE */}
                  <td className="p-2.5 overflow-hidden text-left">
                  ...
                  {/* EST. VALUE */}
                  <td className="p-2.5 overflow-hidden text-left font-mono font-semibold text-ai-purple">
                    {fmtINR(c.estValue)}
                  </td>

                  {/* ACTION */}
                  <td className="p-2.5 overflow-hidden text-center">
                    <div className="flex flex-nowrap justify-left gap-1">
                      <button
                        type="button"
                        onClick={(e) => {
                          e.stopPropagation();
                        ...
                        className="cursor-pointer whitespace-nowrap rounded-[5px] border-none bg-[linear-gradient(135deg,#FF4757_0%,#A8222F_100%)] px-2.5 py-[3px] text-[10px] font-bold text-white"
                      >
                        ⚠ Escalate
                      </button>
                      <button
                        ...
                        className="cursor-pointer whitespace-nowrap rounded-[5px] border border-border bg-transparent px-2.5 py-[3px] text-[10px] font-semibold text-text-mid"
                      >
                        Reassign
                      </button>
                    </div>
