...
const BASE_CHART_OPTS: ChartOptions = {
  ...
  plugins: {
    legend: {
      display: true,
      position: 'bottom',
      labels: { usePointStyle: true, padding: 10, font: { size: 10, family: 'IBM Plex Sans' } },
    },
    tooltip: {
      backgroundColor: '#fff',
      titleColor: '#1A1A2E',
...
const DRIVER_COLORS = ['#DC3545', '#E6921E', '#17A2B8', '#7C3AED', '#28A745', '#8B95A5']
...
export function RiskProfileTab({ meter }: RiskProfileTabProps) {
  ...
  const cluster = useMemo(() => getSimilarConsumersCluster(meter.id), [meter.id])
  ...
  const driversTableData = useMemo(
    () => ({
      ...
      datasets: [{ label: '% contribution', data: driversData.map((d) => d.pct) }],
    }),
    [driversData],
  )
    ...
    const datasets = [
      {
        ...
        data: risk,
        borderColor: '#DC3545',
        backgroundColor: trendKind === 'area' ? 'rgba(220,53,69,.15)' : isBar ? '#DC3545' : 'rgba(220,53,69,.08)',
        fill: trendKind === 'area',
        tension: isBar ? 0 : 0.3,
        borderWidth: isBar ? 0 : 2.5,
        pointRadius: isBar ? 0 : 3,
        pointBackgroundColor: risk.map((v) => (v >= 70 ? '#DC3545' : v >= 40 ? '#E6921E' : '#28A745')),
    ...
    driversChartRef.current = new Chart(driversCanvasRef.current, {
      ...
      options: {
        ...
        scales: {
          x: {
            grid: { color: '#EDF2F7' },
            ticks: { font: { size: 10, family: 'IBM Plex Sans' }, color: '#8B95A5', callback: (v) => `${v}%` },
          },
          y: { grid: { display: false }, ticks: { font: { size: 10, family: 'IBM Plex Sans' }, color: '#4A5568' } },
        },
      } as ChartOptions,
    })
    ...
    <div>
      ...
      <div className="mb-4 grid gap-4 lg:grid-cols-2">
        ...
        <div className="card mb-0">
          <div className="mb-1 flex items-start justify-between gap-2">
            <div className="card-title text-[13px] font-bold">Risk score — 12 month trend</div>
            <ChartControls
              types={['line', 'area', 'bar']}
              ...
              onTableToggle={() => setTrendView((v) => (v === 'table' ? 'chart' : 'table'))}
              onDownload={() => downloadChartPng(trendChartRef.current, `VidyutRaksha_riskTrend_${meter.id}.png`)}
            />
          </div>
          ...
          {trendView === 'chart' ? (
            ...
            <ChartTableView
              data={trendTableData}
              onCopyCSV={() => exportChartCSV(trendTableData, `VidyutRaksha_riskTrend_${meter.id}.csv`)}
            />
          )}
        ...
        <div className="card mb-0">
          <div className="mb-1 flex items-start justify-between gap-2">
            <div className="card-title text-[13px] font-bold">Risk drivers — rule breakdown</div>
            <ChartControls
              types={['bar', 'line', 'area']}
              ...
              onTableToggle={() => setDriversView((v) => (v === 'table' ? 'chart' : 'table'))}
              onDownload={() => downloadChartPng(driversChartRef.current, `VidyutRaksha_riskDrivers_${meter.id}.png`)}
            />
          </div>
          <div className="mb-3 text-[10.5px] text-text-dim">
          ...
          {driversView === 'chart' ? (
            ...
            <ChartTableView
              ...
              onCopyCSV={() => exportChartCSV(driversTableData, `VidyutRaksha_riskDrivers_${meter.id}.csv`)}
            />
          )}
      ...
      {cluster && (
        <div className="card mt-4">
          <div className="card-title text-[13px] font-bold" style={{ color: 'var(--ai-purple)' }}>
            ✦ Similar consumers under same DTR (AI-detected cluster)
          </div>
          <div className="mb-2.5 mt-1 text-[11px] text-text-mid">
            AI has identified <strong>{cluster.consumers.length} other consumers</strong> under{' '}
            {cluster.dtrName} showing the same consumption signature. This suggests{' '}
            <strong>coordinated theft</strong> rather than an isolated case. Recommended: batch inspection
            of all {cluster.consumers.length + 1} consumers on the same day.
          </div>
                ...
                {cluster.consumers.map((c) => {
                  ...
                  return (
                    <tr key={c.meter} className="table-row">
                      ...
                      <td className="font-mono text-text-mid">{c.tamperEvents}</td>
                      <td>
                        <div
                        ...
                        <span
                          ...
                          style={
                            c.status === 'In Progress'
                              ...
                              : { background: 'rgba(27,114,232,0.1)', color: '#1b72e8' }
                          }
                        >
                          {c.status}
                        </span>
