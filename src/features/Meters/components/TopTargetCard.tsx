...
export function TopTargetCard({ meter: m, rank }: TopTargetCardProps) {
  ...
  const riskColor = m.risk >= 80 ? '#DC3545' : m.risk >= 60 ? '#E6921E' : 'var(--amber-dark)'
  const riskBg = m.risk >= 80 ? 'rgba(220,53,69,0.08)' : m.risk >= 60 ? 'rgba(230,146,30,0.08)' : 'rgba(180,117,24,0.08)'
  ...
  return (
    <div
      className="group flex cursor-pointer items-center gap-3 rounded-lg border border-border-light p-[11px_14px] transition-all duration-150 hover:-translate-x-0 hover:translate-x-[2px] hover:shadow-[0_4px_12px_rgba(0,0,0,0.06)]"
      style={{
    ...
    >
      ...
      <div className="shrink-0 text-right">
        <div className="text-[9.5px] font-bold uppercase tracking-[0.4px] text-text-dim">Est. recovery</div>
        <div className="font-mono text-[15px] font-extrabold" style={{ color: 'var(--green)' }}>
          {fmtINR(m._exposure)}
        </div>
