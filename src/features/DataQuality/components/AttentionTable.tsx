...
export function AttentionTable({ rows, title }: AttentionTableProps) {
  return (
    <div className="card" style={{ marginBottom: 14 }}>
      <div className="card-title dq-title-red">{title}</div>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.area} className="table-row">
                  <td className="dq-area">{r.area}</td>
                  <td className="dq-mono" style={{ color: r.countColor }}>
                    {r.stopped}
                  </td>
                  <td className="dq-mono">{r.pct}</td>
                  <td className="dq-mono-dim">{r.lastComm}</td>
                  <td>
